...
export function RetraitiaReport({ report, letters, diagnosticId }: ReportProps) {
  ...
  const [activeLetter, setActiveLetter] = useState<string>('reclamation_carsat')

  const handleGenerateLetters = async () => {
    setLoadingLetters(true)
    try {
      const res = await fetch('/api/retraitia/generate-letters', {
        method: 'POST',
      ...
      <div className="max-w-[800px] mx-auto px-6">
        ...
        <div className="text-center mb-8">
          ...
          <h2 className="font-heading text-[clamp(24px,4vw,36px)] font-bold text-slate-text">
            {report?.title || "Rapport d'audit — Pension de retraite"}
          </h2>
        ...
        {fs && (
          <div className="bg-emerald/5 border border-emerald/20 rounded-2xl p-6 mb-8">
            <h3 className="font-heading font-bold text-slate-text text-base mb-4">💰 Bilan financier — Impact sur votre pension</h3>
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 mb-4">
              <div>
                <div className="text-xs text-slate-muted">Pension déclarée</div>
                <div className="font-bold text-slate-text">{fs.pension_declared_monthly}€/mois</div>
              </div>
              <div>
                <div className="text-xs text-slate-muted">Impact mensuel</div>
                <div className="font-bold text-emerald">{fs.impact_monthly_min}–{fs.impact_monthly_max}€/mois</div>
              </div>
              <div>
                <div className="text-xs text-slate-muted">Impact annuel</div>
                <div className="font-bold text-emerald">{fs.impact_annual_min}–{fs.impact_annual_max}€/an</div>
              </div>
            </div>
            <div className="border-t border-emerald/20 pt-4 flex items-center justify-between flex-wrap gap-2">
              <span className="font-semibold text-slate-text">Impact total sur espérance de vie ({fs.life_expectancy_years} ans)</span>
              <span className="font-heading text-[28px] font-extrabold text-emerald">
                {fmt(fs.impact_lifetime_min || 0)}–{fmt(fs.impact_lifetime_max || 0)}€
              </span>
            </div>
          </div>
        )}
          ...
          <div>
            ...
            <div className="flex gap-2 mb-4 flex-wrap">
              {[
                ...
                { key: 'saisine_cra', label: '🏛️ Recours CRA' },
                { key: 'saisine_mediateur', label: '⚖️ Médiateur' },
              ].map(tab => (
                <button
                  key={tab.key}
            ...
            <div className="mt-4 p-3 bg-amber-50 border border-amber-200 rounded-xl text-xs text-amber-700">
              ⚠️ Complétez les champs entre [crochets] avec vos informations personnelles avant envoi. Le n° de Sécurité Sociale sera nécessaire pour vos courriers.
            </div>
