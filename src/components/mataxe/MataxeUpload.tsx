...
interface MataxeUploadProps {
  onExtractionComplete: (extraction: MataxeExtractionResult) => void
  onManualMode: () => void
}
...
export function MataxeUpload({ onExtractionComplete, onManualMode }: MataxeUploadProps) {
  ...
  const extractDocuments = async (forceVision = false, visionConsent = false) => {
    setState('extracting')
    setError(null)
    track({ event: 'upload_completed', brique: 'mataxe', file_count: files.length })
    ...
    try {
      ...
      const res = await fetch('/api/mataxe/extract', { method: 'POST', body: formData })
      ...
      if (!data.success) {
        if (data.needsVisionConsent) {
          ...
          track({ event: 'ocr_low_confidence', brique: 'mataxe', confidence: data.ocrConfidence })
          return
        }
      ...
      track({ event: 'extraction_success', brique: 'mataxe', mode: data.extraction.mode })
      setState('done')
  ...
  const handleVisionConsent = () => {
    extractDocuments(true, true)
  }
      ...
      <div className="max-w-[680px] mx-auto px-6">
        <div className="text-center mb-10">
          <h2 className="font-heading text-[clamp(24px,4vw,36px)] font-bold text-slate-text mb-3">
            Déposez votre formulaire 6675-M
          </h2>
          <p className="text-slate-muted text-base max-w-[520px] mx-auto">
            Notre IA extrait automatiquement les données cadastrales pour un diagnostic à <strong className="text-emerald">95% de fiabilité</strong>.
          </p>
        </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 mb-6">
          {[
            { icon: '📋', title: 'Formulaire 6675-M', desc: 'Fiche d\'évaluation cadastrale', highlight: true },
            { icon: '📄', title: 'Avis de taxe foncière', desc: 'Dernier avis reçu', highlight: false },
            { icon: '🗺️', title: 'Relevé de propriété', desc: 'Extrait cadastral', highlight: false },
          ].map((d) => (
        ...
        {state === 'extracting' && (
          <div className="p-8 bg-white rounded-2xl border border-emerald/20 text-center">
            ...
            <p className="text-base font-semibold text-slate-text mb-3">Analyse de vos documents...</p>
            <div className="space-y-2">
              {['Lecture du formulaire (OCR)', 'Extraction des paramètres cadastraux', 'Identification des équipements', 'Calcul de la surface pondérée'].map((msg, i) => (
          ...
          <div className="p-6 bg-yellow-50 rounded-2xl border border-yellow-200">
            ...
            <div className="p-3 bg-yellow-100/50 rounded-xl mb-4">
              <p className="text-xs text-yellow-800">
                ⚠️ En mode Vision, les images de vos documents sont envoyées à notre IA pour une lecture visuelle directe.
                Les données personnelles (nom, adresse) sont anonymisées dans le rapport final.
              </p>
            </div>
            <div className="flex gap-3">
              <button
                onClick={handleVisionConsent}
        ...
        {(state === 'idle' || state === 'error') && (
          <div className="text-center mt-8">
            <button onClick={onManualMode} className="text-sm text-emerald-dark hover:text-emerald font-medium transition-colors">
              Je n&apos;ai pas le 6675-M — remplir manuellement →
            </button>
          </div>
        )}

        {/* Aide pour obtenir le 6675-M */}
        {(state === 'idle' || state === 'error') && (
          <div className="mt-6 p-4 bg-navy/[0.03] rounded-xl border border-navy/10">
            <p className="text-xs font-semibold text-slate-text mb-1">💡 Comment obtenir votre 6675-M ?</p>
            <p className="text-xs text-slate-muted leading-relaxed">
              Connectez-vous sur <strong>impots.gouv.fr</strong> → Messagerie sécurisée → &laquo;Je signale une erreur sur le calcul de mon impôt&raquo; → Demandez votre fiche d&apos;évaluation cadastrale (formulaire 6675-M). Réponse sous 2 à 4 semaines.
            </p>
          </div>
        )}
