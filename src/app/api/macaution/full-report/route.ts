// ============================================================
// POST /api/macaution/full-report
...
import { FULL_REPORT_SYSTEM_PROMPT, buildFullReportUserMessage } from '@/lib/macaution/prompts'
...
export async function POST(request: NextRequest) {
  try {
    ...
    // 2. Recalculer (les calculs sont déterministes)
    ...
    try {
      ...
      console.error('[MACAUTION] Erreur rapport complet:', aiError instanceof Error ? aiError.message : aiError)
    ...
    try {
      await payload.create({
        collection: 'reports',
        data: {
          title: `Rapport MACAUTION — ${new Date().toLocaleDateString('fr-FR')}`,
      ...
      console.error('[MACAUTION] Erreur sauvegarde rapport:', dbError instanceof Error ? dbError.message : dbError)
    }

    return NextResponse.json({ success: true, report })

  } catch (error) {
    console.error('[MACAUTION] Erreur inattendue full-report:', error)
