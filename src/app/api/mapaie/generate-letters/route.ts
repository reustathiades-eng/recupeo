// POST /api/mapaie/generate-letters — Courriers LRAR employeur + saisine CPH (payant 129€ — plan TWELVE_MONTHS)
...
import { RECLAMATION_PROMPT, buildReclamationMessage } from '@/lib/mapaie/prompts'
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Vérification du paiement Stripe (plan 129€ — TWELVE_MONTHS inclut la réclamation)
    ...
    if (payment.plan !== 'TWELVE_MONTHS') {
      return NextResponse.json(
        { success: false, error: 'Les courriers de réclamation nécessitent le plan Audit 12 mois' },
        { status: 403 }
      )
    }
    ...
    if (anomalies.length === 0) {
      return NextResponse.json(
        { success: false, error: 'Aucune anomalie détectée — courriers non générés' },
        { status: 422 }
      )
    }

    const totalMensuel = anomalies.reduce((s, a) => s + (a.calculation?.montantMensuel ?? 0), 0)
    const rappel = calculerRappelSur3Ans(totalMensuel, emploi.dateEntree)

    // Appel Claude — courriers de réclamation
    const userMessage = buildReclamationMessage(
      'Employeur',
    ...
    const claudeResponse = await callClaude({
      system: RECLAMATION_PROMPT,
      userMessage,
      maxTokens: 6144,
      temperature: 0.3,
    })

    let letters: { lrarEmployeur: string; saisiineCPH: string; guideProcedure: string }
    try {
      ...
      letters = {
        lrarEmployeur: raw.lrar_employeur ?? '',
        saisiineCPH: raw.saisine_cph ?? '',
        guideProcedure: raw.guide_procedure ?? '',
      }
    } catch {
      letters = {
        lrarEmployeur: claudeResponse,
        saisiineCPH: '',
        guideProcedure: '',
      }
    }
    ...
    return NextResponse.json({
      ...
      letters,
      anomalieCount: anomalies.length,
      montantReclame: {
        brut: rappel.montantTotalBrut,
        net: rappel.montantNetEstime,
        moisConcernes: rappel.moisConcernes,
      },
    })
  } catch (err) {
    console.error('[mapaie-generate-letters] Erreur:', err)
    return NextResponse.json(
